...
// // In-memory store for pipeline / CI status per repo
// // key: repoFullName (e.g. "ZenMachina16/test")
...
//       // Update in-memory pipeline status for this repo
//       pipelineStatus[repoFullName] = {
//         status: workflow_run.conclusion === 'success' ? 'success' : 'failure',
//         conclusion: workflow_run.conclusion,
//         lastEvent: 'workflow_run_completed',
...
// /**
//  * GET /api/github/pipeline/status?repo=owner/name
//  * Return last known CI/pipeline status for a given repo
//  */
// router.get('/pipeline/status', (req, res) => {
//   const repo = req.query.repo; // e.g. "ZenMachina16/test"

//   if (!repo) {
//     return res.status(400).json({ error: 'Missing repo query parameter (owner/name).' });
//   }

//   const status = pipelineStatus[repo];

//   if (!status) {
//     return res.json({
//       repo,
//       status: 'unknown',
//       message: 'No workflow_run status recorded yet.'
//     });
//   }
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    // Handle workflow run completion
    if (eventType === 'workflow_run' && action === 'completed' && workflow_run && repository) {
      const repoFullName = repository.full_name;
      ...
      // 🔹 NEW: If CI succeeded, trigger Deploy Agent (n8n)
      if (workflow_run.conclusion === 'success') {
        await triggerDeployWorkflow(repository, workflow_run);
      }
    }

    // Handle pull request events (for tracking PR creation)
    if (eventType === 'pull_request' && action === 'opened' && pull_request && repository) {
      await handlePullRequestCreated(pull_request, repository);
    }
...
async function handleWorkflowRunCompletion(workflow_run, repository) {
  try {
    const { conclusion, status } = workflow_run;
    const repoFullName = repository.full_name;
    ...
    // Log successful deployments
    if (conclusion === 'success') {
      console.log(`✅ CI pipeline successful for ${repoFullName}`);
      // Additional success-side effects could go here
    }
...
/**
 * Trigger retry workflow in n8n when deployment/CI fails
 */
async function triggerRetryWorkflow(repository, workflow_run) {
  try {
    const n8nWebhookUrl = process.env.N8N_RETRY_WEBHOOK_URL;
    if (!n8nWebhookUrl) {
      console.warn('N8N_RETRY_WEBHOOK_URL is not set, skipping retry trigger.');
      return;
    }
    ...
    console.log('Triggering retry workflow for failed deployment/CI...');
...
/**
 * 🔹 NEW: Trigger deploy workflow in n8n when CI succeeds
 */
async function triggerDeployWorkflow(repository, workflow_run) {
  try {
    const n8nDeployWebhookUrl = process.env.N8N_DEPLOY_WEBHOOK_URL;
    if (!n8nDeployWebhookUrl) {
      console.warn('N8N_DEPLOY_WEBHOOK_URL is not set, skipping deploy trigger.');
      return;
    }

    // GHCR image ref pattern: ghcr.io/<owner>/<repo>:<head_sha> (all lowercase)
    const owner = repository.owner.login;
    const repo = repository.name;
    const ownerLower = owner.toLowerCase();
    const repoLower = repo.toLowerCase();
    const imageRef = `ghcr.io/${ownerLower}/${repoLower}:${workflow_run.head_sha}`;
    ...
    const payload = {
      ...
      image: {
        ref: imageRef,
        registry: 'ghcr.io',
        repository: `${ownerLower}/${repoLower}`,
        tag: workflow_run.head_sha,
      },
      deploy: {
        environment: 'staging',
        triggered_at: new Date().toISOString(),
        trigger_reason: 'ci_success',
      },
    };

    console.log('Triggering deploy workflow in n8n...', { imageRef });

    const response = await axios.post(n8nDeployWebhookUrl, payload, {
      headers: { 'Content-Type': 'application/json' },
      timeout: 30000,
    });

    console.log('Deploy workflow triggered successfully:', response.status);
  } catch (error) {
    console.error('Failed to trigger deploy workflow:', error.message || error);
  }
}
...
router.get('/webhook/status', (req, res) => {
  res.json({
    ...
    retryWebhookUrl: process.env.N8N_RETRY_WEBHOOK_URL,
    deployWebhookUrl: process.env.N8N_DEPLOY_WEBHOOK_URL,
    timestamp: new Date().toISOString(),
  });
});
